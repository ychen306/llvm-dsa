...
INITIALIZE_PASS_BEGIN(LocalDataStructures, "", "", true, true)
INITIALIZE_PASS_DEPENDENCY(AddressTakenAnalysis)
INITIALIZE_PASS_END(LocalDataStructures, "", "", true, true)
...
void GraphBuilder::visitGetElementPtrInst(User &GEP) {
  ...
  for (gep_type_iterator I = gep_type_begin(GEP), E = gep_type_end(GEP);
       I != E; ++I)
    if (StructType *STy = dyn_cast<StructType>(I.getIndexedType())) {
      ...
      if (TypeInferenceOptimize) {
        if (ArrayType* AT = dyn_cast<ArrayType>(STy->getTypeAtIndex(FieldNo))) {
          ...
          while (isa<ArrayType>((++J).getIndexedType())) {
